"""
raise_gamma_sensitivity.py
==========================
Sensibilité au gamma pour SD-VAE L11 sur RAISE.
...
Protocole :
  ...
  - Gammas testés (tous dérivés de l'ancrage) :
      ...
      • γ_mean     = 1 / (2 · mean(d²))
      • 10×median, 50×median, 100×median

Usage :
    .venv/bin/python experiments/raise_gamma_sensitivity.py
"""
...
OUT_DIR             = BASE_DIR / "results" / "raise_gamma_sensitivity"
...
def compute_anchor_gammas(D_rr: np.ndarray) -> dict:
    """
    Calcule les gammas heuristiques à partir des paires de l'ancrage.

    Retourne un dict : tag → gamma (float)
    """
    triu = D_rr[np.triu_indices(D_rr.shape[0], k=1)]
    median_d2 = float(np.median(triu))
    mean_d2   = float(np.mean(triu))

    gamma_median = 1.0 / (2.0 * median_d2 + 1e-8)
    gamma_mean   = 1.0 / (2.0 * mean_d2   + 1e-8)
    ...
    gammas = {
        ...
        "100x_median":100.0 * gamma_median,
    }

    logger.info(f"  median(d²) = {median_d2:.4e}  → γ_median  = {gamma_median:.4e}")
    logger.info(f"  mean(d²)   = {mean_d2:.4e}  → γ_mean    = {gamma_mean:.4e}")
...
def main():
    ...
    for gamma_tag, gamma_val in gammas.items():
        ...
        label = f"SD_VAE_L{LAYER}_{gamma_tag}"
    ...
    # ── Sauvegarde CSV ────────────────────────────────────────────────
    tag = f"sdvae_L{LAYER}_{N_GROUPS}x{GROUP_SIZE}"
