...
=========================
Figures pour raise_gamma_sensitivity.py (SD-VAE L11, 24×20, 5 gammas).

Figures produites :
  01_barplot_neg_rho.png  — barplot −ρ(Spearman) pour les 5 gammas
  02_scatters_mmd_mos.png — grille 5 scatters (MMD vs MOS), 1 par gamma,
                            avec droite de régression linéaire et annotation
                            −ρ + p-value

Usage :
    .venv/bin/python experiments/plot_gamma_sensitivity.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "raise_gamma_sensitivity"
...
# Ordre des gammas (du plus petit au plus grand)
GAMMA_ORDER = ["median", "mean", "10x_median", "50x_median", "100x_median"]
GAMMA_LABELS = {
    "median":      "γ_median",
    "mean":        "γ_mean",
    "10x_median":  "10 × γ_median",
    "50x_median":  "50 × γ_median",
    "100x_median": "100 × γ_median",
}
...
# Couleurs par gamma (dégradé du bleu clair au foncé)
GAMMA_COLORS = {
    "median":      "#1565C0",
    "mean":        "#42A5F5",
    "10x_median":  "#FF9800",
...
# ═══════════════════════════════════════════════════════════════════════
#  Figure 1 — Barplot −ρ(Spearman) pour les 5 gammas
# ═══════════════════════════════════════════════════════════════════════

fig1, ax1 = plt.subplots(figsize=(10, 5))
ax1.set_title(
    f"SD-VAE L11 — Sensibilité au gamma  |  RAISE {n_groups} groupes × 20 images  |  ancrage 1000 COCO\n"
...
# ═══════════════════════════════════════════════════════════════════════
#  Figure 2 — Grille 5 scatters (MMD vs MOS)
# ═══════════════════════════════════════════════════════════════════════

fig2, axes = plt.subplots(1, 5, figsize=(22, 5))
fig2.suptitle(
    f"SD-VAE L11 — MMD² vs MOS  |  RAISE {n_groups} groupes × 20 images  |  ancrage 1000 COCO\n"
