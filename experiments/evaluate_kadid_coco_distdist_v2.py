"""
evaluate_kadid_coco_distdist_v2.py
====================================
KADID-10k  —  distribution-vs-distribution MMD²
avec 1000 images COCO comme ancrage.  VERSION 2 — nouvelles couches.

Backbones : SD-VAE L13 (encoder.mid_block.attentions.0)
            DC-AE L12  (encoder.down_blocks.3.2)
            DINOv2 L5  (blocks.4)
Gammas    : γ_med/10, γ_med, 10×γ_med
Référence : CMMD (CLIP ViT-L/14@336) avec γ_med/10, γ_med, 10×γ_med

Résultats dans : results/kadid_coco_distdist_v2/
(Les résultats v1 dans kadid_coco_distdist/ restent intacts.)

Usage :
    .venv/bin/python experiments/evaluate_kadid_coco_distdist_v2.py
    .venv/bin/python experiments/evaluate_kadid_coco_distdist_v2.py --skip-cmmd
"""
...
OUT_DIR    = BASE_DIR / "results" / "kadid_coco_distdist_v2"
...
# SD-VAE L13  = encoder.mid_block.attentions.0
# DC-AE L12   = encoder.down_blocks.3.2
# DINOv2 L5   = blocks.4
BACKBONE_CONFIGS = [
    ("sd_vae",        13, "SD-VAE L13",   4),
    ("dc_ae",         12, "DC-AE L12",    2),
...
# CMMD gamma tags to include in results
CMMD_TAGS = {"div10", "median", "10x", "fixed"}
...
def plot_scatter(all_results, out_dir: Path):
    ...
    _offsets = {
        "GMMD DINOv2 L5 γ_med":        (14, -18),
        "GMMD SD-VAE L13 γ_med/10":     (-60, -25),
        "CMMD γ=0.005":                 (14, 14),
    }
    ...
    for entry in all_results:
        ...
        annotate = False
        if g_tag == "median" and bname not in labeled_backbones and bname != "CMMD" and bname != "SD-VAE L13":
            annotate = True
            labeled_backbones.add(bname)
        elif entry["label"] == "GMMD SD-VAE L13 γ_med/10":
            annotate = True
        elif entry["label"] == "CMMD γ=0.005":
            annotate = True
...
def plot_tripanel(all_results, out_dir: Path):
    sorted_res = sorted(all_results,
                        key=lambda x: (-x["stats"]["spearman"], -x["stats"]["kendall"]),
                        reverse=True)
