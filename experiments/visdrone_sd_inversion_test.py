"""
visdrone_sd_inversion_test.py
==============================
Anchor  : 1000 images VisDrone originales
Query A : 1000 images VisDrone SD img2img (strength=0.45)
...
SD_DIR       = BASE_DIR / "dataset" / "visdrone_sd_styled_s45"
...
def main():
    ...
    # Anchor : VisDrone originaux (tous, pas de filtrage)
    anchor_paths = sample_paths(list(VISDRONE_DIR.glob("*.jpg")), N_ANCHOR, SEED)
    sd_paths     = sample_paths(list(SD_DIR.glob("*.png")), N_QUERY, SEED+1)
    ...
    logger.info(f"Query A : {len(sd_paths)} VisDrone SD img2img (strength=0.45)")
    ...
    logger.info("Extraction Gram SD styled...")
    sd_std = get_gram(sd_paths, ext_std)
    sd_raw = get_gram(sd_paths, ext_raw)
    ...
    gram_std_sd   = {t: mmd2_unbiased(anchor_std, sd_std,   g_std*m) for t, m in GAMMA_MULTIPLIERS.items()}
    gram_std_coco = {t: mmd2_unbiased(anchor_std, coco_std, g_std*m) for t, m in GAMMA_MULTIPLIERS.items()}
    gram_raw_sd   = {t: mmd2_unbiased(anchor_raw, sd_raw,   g_raw*m) for t, m in GAMMA_MULTIPLIERS.items()}
    ...
    sd_clip     = get_clip_feats(sd_paths,     clip_model, clip_preprocess, device)
    coco_clip   = get_clip_feats(coco_paths,   clip_model, clip_preprocess, device)

    g_clip = median_gamma(anchor_clip)
    cmmd_sd_fixed   = mmd2_unbiased(anchor_clip, sd_clip,   CMMD_GAMMA)
    cmmd_coco_fixed = mmd2_unbiased(anchor_clip, coco_clip, CMMD_GAMMA)
    cmmd_sd_med     = mmd2_unbiased(anchor_clip, sd_clip,   g_clip)
    cmmd_coco_med   = mmd2_unbiased(anchor_clip, coco_clip, g_clip)

    sep = "=" * 75
    logger.info(f"\n{sep}")
    logger.info("VISDRONE SD IMG2IMG INVERSION TEST (strength=0.45)")
    logger.info(f"  Anchor: VisDrone orig | Query A: VisDrone SD | Query B: COCO")
    logger.info(sep)
    logger.info(f"  {'Méthode':<22} {'SD styled':>14}  {'COCO':>14}  {'Ratio':>7}  INV?")
    logger.info(f"  {'-'*68}")
    for tag in GAMMA_MULTIPLIERS:
        print_row(f"Gram_std {tag}", gram_std_sd[tag], gram_std_coco[tag])
    for tag in GAMMA_MULTIPLIERS:
        print_row(f"Gram_raw {tag}", gram_raw_sd[tag], gram_raw_coco[tag])
    print_row("CMMD γ=0.005", cmmd_sd_fixed, cmmd_coco_fixed)
    print_row("CMMD γ_med",   cmmd_sd_med,   cmmd_coco_med)
    logger.info(sep)

    cmmd_inv = cmmd_sd_fixed < cmmd_coco_fixed or cmmd_sd_med < cmmd_coco_med
    gram_inv_any = (any(gram_std_sd[t] < gram_std_coco[t] for t in GAMMA_MULTIPLIERS) or
                    any(gram_raw_sd[t] < gram_raw_coco[t] for t in GAMMA_MULTIPLIERS))
    gram_inv_small_g = (gram_std_sd["γ/10"] < gram_std_coco["γ/10"] or
                        gram_raw_sd["γ/10"] < gram_raw_coco["γ/10"])

    if cmmd_inv and not gram_inv_small_g:
        logger.info("  ★ SUCCÈS : CMMD inverse, Gram-MMD (γ/10) ne s'inverse PAS ★")
