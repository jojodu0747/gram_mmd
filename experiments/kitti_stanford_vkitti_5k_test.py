"""
kitti_stanford_vkitti_5k_test.py
=================================
Anchor  : 5000 images KITTI originales (dataset/kitti/images/)
Query A : 1000 images VKITTI2 variées (toutes conditions, tous scènes)
Query B : 1000 images Stanford Cars (voitures réelles, même sémantique)
...
def main():
    ...
    logger.info(f"Query B : {len(stanford_paths)} Stanford Cars (pool={len(stanford_all)})")
    ...
    out_path = RESULTS_DIR / f"kitti_stanford_vkitti_5k_{ts}.json"

    results = {
        "experiment": "kitti_stanford_vkitti_5k_test",
        "timestamp": ts,
        "config": {
